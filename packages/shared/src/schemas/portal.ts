...
// ─── Inputs ──────────────────────────────────────────────────────────────────
export const CreateNegotiationInputSchema = z.object({
  lineId: z.string().nullable().optional(),
  comment: z.string().optional(),
  counterDiscountPct: z.number().min(0).max(100).optional(),
}).refine(
  (data) => (data.comment && data.comment.trim().length > 0) || data.counterDiscountPct !== undefined,
  { message: "Either a comment or a target discount percentage is required." },
);
export type CreateNegotiationInput = z.infer<typeof CreateNegotiationInputSchema>;

export const AnswerNegotiationInputSchema = z.object({
  status: z.enum(["ACCEPTED", "ANSWERED"]),
  repComment: z.string().optional(),
});
export type AnswerNegotiationInput = z.infer<typeof AnswerNegotiationInputSchema>;
